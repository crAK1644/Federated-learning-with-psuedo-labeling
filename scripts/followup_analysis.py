...
def emit_memo(q1: dict, q2: dict, q6: dict) -> str:
    ...
    parts = [
        ...
        r"\author{Supplementary analysis to \texttt{report.pdf}}",
        r"\date{Generated by \texttt{scripts/followup\_analysis.py}}",
        r"\begin{document}", r"\maketitle",

        r"\section*{Summary}",
        "One finding changes the reading of the main report: the gafgyt.tcp / gafgyt.udp merge "
        "is not primarily a voting-protocol failure. After scaling, the two classes are not "
        "merely close in feature space -- each has collapsed onto a single point, so the "
        "federation's clients cannot distinguish them at all. Everything downstream (the "
        "precision gap, the useless validity mask, the failure of stricter voting rules) follows "
        "from that. The remaining sections cover the precision arithmetic, the pseudo-label "
        "validity rate, the byte-accounting conventions, and the traffic scope of Figure 17; "
        "known limitations are collected at the end.",
        ...
        r"\textbf{The degeneracy is manufactured by the scaler, not present in N-BaIoT.} The raw "
        r"CSV tree, re-downloaded from the UCI archive and counted directly, is richly distinct:",
        r"\begin{quote}\small "
        ...
        r"\section{Voting and confidence thresholds under a controlled test}",
        "The main report's closing claim -- that hard labels plus majority voting discard the "
        "uncertainty that would let the federation recover a lost class -- assumes the clients "
        "hold that uncertainty in the first place. Section 1 shows they do not: the two classes "
        "are one point each, so there is no uncertainty signal being thrown away.",
        "",
        r"\textbf{Hypothesis.} The collapse originates in feature preparation, not in the decision "
        r"rule. \textbf{Prediction.} No confidence-threshold and no vote-margin rule recovers both "
        r"flooding classes; only a non-affine normalisation can.",
        "",
        r"The server recorded the full $8900 \times 11$ vote matrix for every one of the 200 "
        r"rounds in all three scenarios, so any decision rule can be re-scored offline against "
        r"open-split ground truth without retraining. Eight rules were tested: the shipped plain "
        r"majority, margins of 1, 2, 3 and 5 votes, and quorums of 0.50, 0.60 and 0.75. Every "
        r"stricter rule behaves the same way -- it converts confident wrong labels into "
        r"abstentions and leaves the collapsed class on the floor. Figure F-A6 shows why: at the "
        r"rounds where the merge locks in, the wrong consensus is close to unanimous, so there is "
        r"no thin margin for a veto to catch.",
        ...
        r"A closed-loop version of the same comparison is specified in "
        r"\texttt{configs/experiments\_collapse.yaml} -- baseline, fixed confidence thresholds of "
        r"0.70/0.80/0.90, soft labels with voting disabled, a vote margin of 2, and the quantile "
        r"dataset, over two scenarios and three seeds -- and is committed and runnable, but was "
        r"not executed for this note. The prediction it would test is already sharp: six of the "
        r"seven arms act on the decision rule or apply an affine rescale, and should fail; the "
        r"quantile arm is the only one whose mechanism reaches the measured cause.",

        r"\section{Limitations}",
        r"\begin{itemize}",
        r"\item The re-voting analysis is \emph{open-loop}: it re-scores ballots that were "
        r"recorded under the shipped rule. A different rule would also have changed what the "
        r"clients trained on in later rounds, so it measures the effect of a rule at round $r$, "
        r"not the end state of a different protocol.",
        r"\item The quantile evidence is a linear probe on the prepared dataset plus a two-round "
        r"pipeline check, not a federated training result. Whether training actually recovers "
        r"both flooding classes under the rank transform is untested; the prediction is "
        r"falsifiable but unfalsified. Should it fail despite the pair being separable at 0.9892, "
        r"the protocol explanation returns to the table.",
        r"\item 0.9892 is a ceiling, not a shortfall of the transform: 38 gafgyt.tcp rows are "
        r"bit-identical to the gafgyt.udp vector in the source CSVs and cannot be recovered by "
        r"any per-feature transform.",
        r"\item The local dataset copy was verified against the one the runs used by per-file "
        r"checksum (every feature and label array is byte-identical); the manifest hashes "
        r"themselves differ, in audit and plot files only.",
        r"\item All counts are on the mini-N-BaIoT sampling used throughout the reproduction. "
        r"The float32 argument depends only on the ratio of the class gap to the global feature "
        r"range and so carries over to the full dataset, but the exact unique-row counts do not.",
        r"\end{itemize}",
        r"\end{document}", ""]
    return "\n".join(parts)
